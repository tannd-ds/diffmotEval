...
def iou_distance_multi(atracks, btracks):
    """
    Compute cost based on IoU
...
def gtidiou_distance(atracks, btracks):
    """
    Compute cost based on IoU
    :type atracks: list[STrack]
    :type btracks: list[STrack]
    ...
    #     btlbrs = btracks
    # else:
    #     atlbrs = [track.gt_id for track in atracks]
    #     btlbrs = [track.gt_id for track in btracks]
    if len(atracks) == 0 or len(btracks) == 0:
        return np.ones((len(atracks), len(btracks)))

    atlbrs = [track.gt_id for track in atracks]
    btlbrs = [track.gt_id for track in btracks]
    cost_matrix = np.ones((len(atlbrs), len(btlbrs)))
    for i in range(len(atlbrs)):
        for j in range(len(btlbrs)):
            if atlbrs[i] == btlbrs[j]:
                cost_matrix[i, j] = 0
                continue
